...
describe('resolveAgentsSelection', () => {
  ...
  it('throws on missing global name', () => {
    ...
    expect(() => resolveAgentsSelection(resolved, { selectedName: 'missing' })).toThrowError(/AGENTS content not found/)
  })
  ...
  it('errors on conflicting CLI flags', () => {
    ...
    expect(() => resolveAgentsSelection(resolved, { selectedName: 'default', noGlobal: true })).toThrowError(
      /Cannot select a global AGENTS entry and disable global AGENTS/
    )
  })
})
