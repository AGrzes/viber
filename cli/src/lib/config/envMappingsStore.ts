...
function updateMappings(existing: EnvMappingEntry[] | undefined, key: string, value: string): EnvMappingEntry[] {
...
export async function listEnvMappings(scope: EnvMappingScope, cwd = process.cwd()): Promise<EnvMappingEntry[]> {
  if (scope === 'global') {
...
export async function deleteEnvMapping(scope: EnvMappingScope, key: string, cwd = process.cwd()): Promise<void> {
